...
function rowToCompany(row: CompanyRow): Company {
  ...
  return {
    ...
    sizeEstimate: row.size_estimate ?? null,
    headquarters: row.headquarters ?? null,
    linkedinUrl: row.linkedin_url ?? null,
    websiteUrl: row.website_url ?? null,
    // These columns may not exist in older databases before migration
    careersPageUrl: row.careers_page_url ?? null,
    culturePageUrl: row.culture_page_url ?? null,
    glassdoorUrl: row.glassdoor_url ?? null,
    crunchbaseUrl: row.crunchbase_url ?? null,
    foundedYear: row.founded_year ?? null,
    extractedUrlsMetadata,
    overallScore: row.overall_score ?? null,
    researchStatus: (row.research_status as Company['researchStatus']) ?? 'pending',
